...
export default function FeaturedWish({ wish, charity }: FeaturedWishProps) {
  ...
  const amountLeft =
    (wish.quantityNeeded - wish.quantityDonated) * wish.unitPrice;
        ...
        <Card className="shadow-lg border-t-4 border-accent overflow-hidden animate-breathing-glow">
          <CardHeader className="text-center pb-4">
            ...
            <CardDescription className="text-base md:text-lg max-w-3xl mx-auto">
              This wish is so close to being fulfilled! Your contribution can be
              ...
              <div>
                ...
                <div className="mb-4">
                  <div className="flex justify-between items-center mb-1 text-sm text-muted-foreground">
                    <span>
                      {formatCurrency(wish.quantityDonated * wish.unitPrice)}{" "}
                      raised
                    </span>
                    <span className="font-semibold">
                      {formatCurrency(wish.quantityNeeded * wish.unitPrice)}{" "}
                      goal
                    </span>
                  </div>
                  <Progress value={progress} className="h-3" />
                  <p className="text-center text-sm font-medium text-accent mt-2 animate-pulse">
                    Just {formatCurrency(amountLeft)} to go!
                  </p>
                </div>

                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <p className="text-lg font-bold text-primary">
                    {formatCurrency(wish.unitPrice)}{" "}
