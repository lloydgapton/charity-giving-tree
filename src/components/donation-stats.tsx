      ...
      <Card className="shadow-lg border-t-4 border-accent">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-base md:text-lg max-w-2xl mx-auto">
            Every contribution, big or small, adds to a wave of generosity.
            Here's a look at what we've achieved together so far.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 pt-6">
            {statItems.map((item) => (
              <div
                key={item.title}
                className="group relative flex flex-col items-center text-center p-6 bg-background rounded-lg border-2 border-transparent transition-all duration-300 hover:border-primary hover:scale-105"
              >
